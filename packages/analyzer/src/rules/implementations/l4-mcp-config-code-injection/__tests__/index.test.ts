/**
 * L4 v2 — structural config-parse tests.
 *
 * Assertions:
 *   - TP fixtures produce at least one finding with the expected primitive kind;
 *   - TN fixtures produce zero findings;
 *   - every link carries a structured Location;
 *   - every VerificationStep.target is a Location;
 *   - confidence is in (0.30, 0.85];
 *   - the threat reference cites a known L4 CVE.
 */

import { describe, it, expect } from "vitest";
import { readFileSync, readdirSync } from "node:fs";
import { join, dirname } from "node:path";
import { fileURLToPath } from "node:url";
import { MCPConfigCodeInjectionRule } from "../index.js";
import type { AnalysisContext } from "../../../../engine.js";
import { isLocation } from "../../../location.js";

const HERE = dirname(fileURLToPath(import.meta.url));
const FIXTURES_DIR = join(HERE, "..", "__fixtures__");

function makeContext(file: string, text: string): AnalysisContext {
  return {
    server: { id: "srv-1", name: "test-server", description: null, github_url: null },
    tools: [],
    source_code: text,
    source_files: new Map([[file, text]]),
    dependencies: [],
    connection_metadata: null,
  };
}

function loadFixture(name: string): { file: string; text: string } {
  const file = join(FIXTURES_DIR, name);
  return { file, text: readFileSync(file, "utf8") };
}

const rule = new MCPConfigCodeInjectionRule();

describe("L4 — MCP Config Code Injection (v2)", () => {
  describe("fires (true positives)", () => {
    it("flags shell-interpreter-command (CVE-2025-59536)", () => {
      const { file, text } = loadFixture("true-positive-01-shell-command.ts");
      const results = rule.analyze(makeContext(file, text));
      expect(results.length).toBeGreaterThan(0);
      const cves = results.map((r) => r.chain.threat_reference?.id);
      expect(cves).toContain("CVE-2025-59536");
    });

    it("flags api-base-env-redirect (CVE-2026-21852)", () => {
      const { file, text } = loadFixture("true-positive-02-api-redirect.ts");
      const results = rule.analyze(makeContext(file, text));
      expect(results.length).toBeGreaterThan(0);
      const cves = results.map((r) => r.chain.threat_reference?.id);
      expect(cves).toContain("CVE-2026-21852");
    });

    it("flags sensitive-env-in-args", () => {
      const { file, text } = loadFixture("true-positive-03-sensitive-env-in-args.ts");
      const results = rule.analyze(makeContext(file, text));
      expect(results.length).toBeGreaterThan(0);
      const cves = results.map((r) => r.chain.threat_reference?.id);
      expect(cves).toContain("CVE-2026-21852");
    });
  });

  describe("does not fire (true negatives)", () => {
    it("ignores a benign npx launcher with safe env", () => {
      const { file, text } = loadFixture("true-negative-01-benign-npx.ts");
      expect(rule.analyze(makeContext(file, text))).toEqual([]);
    });

    it("ignores an unrelated object literal with a command field", () => {
      const { file, text } = loadFixture("true-negative-02-unrelated-object.ts");
      expect(rule.analyze(makeContext(file, text))).toEqual([]);
    });
  });

  describe("chain integrity — v2 contract", () => {
    const tps = readdirSync(FIXTURES_DIR).filter((n) => n.startsWith("true-positive-"));
    for (const name of tps) {
      it(`${name} → every link has a structured Location`, () => {
        const { file, text } = loadFixture(name);
        const results = rule.analyze(makeContext(file, text));
        expect(results.length).toBeGreaterThan(0);
        for (const r of results) {
          for (const link of r.chain.links) {
            if (link.type === "impact") continue;
            expect(isLocation(link.location)).toBe(true);
          }
        }
      });

      it(`${name} → every VerificationStep.target is a Location`, () => {
        const { file, text } = loadFixture(name);
        const results = rule.analyze(makeContext(file, text));
        for (const r of results) {
          const steps = r.chain.verification_steps ?? [];
          expect(steps.length).toBeGreaterThan(0);
          for (const step of steps) {
            expect(isLocation(step.target)).toBe(true);
          }
        }
      });

      it(`${name} → confidence in (0.30, 0.85]`, () => {
        const { file, text } = loadFixture(name);
        const results = rule.analyze(makeContext(file, text));
        for (const r of results) {
          expect(r.chain.confidence).toBeLessThanOrEqual(0.85);
          expect(r.chain.confidence).toBeGreaterThan(0.3);
        }
      });
    }
  });
});
