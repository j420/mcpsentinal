...
class M4ToolSquattingRule implements TypedRuleV2 {
  ...
  private buildFinding(site: SquatSite): RuleResult | null {
    ...
    // Multi-signal-required edge case (CHARTER lethal edge case #4):
    // "trusted" alone is marketing copy. Require either a second signal
    // class, or a bare vendor token, or a non-trust-badge class.
    const classes = new Set(signals.map((s) => s.cls));
    const onlyTrustBadge =
      signals.length > 0 &&
      signals.every((s) => s.id === "trust-badge-trusted");
    if (onlyTrustBadge && site.bare_vendor_token === null) return null;
    ...
    // Suppress the _classes_ unused-var warning by referencing it in the
    // evidence chain builder path.
    void classes;
  ...
  private severityFor(confidence: number): "critical" | "high" | "medium" {
  ...
  private emitFinding(
    ...
    builder.mitigation({
      ...
      detail: site.has_negation
        ? `A negation token was detected near an authenticity anchor. ` +
          `Confidence reduced by 0.70x (multiplicative).`
        : `No negation token found near any authenticity anchor — the claim ` +
          `stands without disclaimer.`,
    });

    const noisyOr = Math.min(0.70, 1 - weights_product_complement(signals));
    builder.factor(
      "noisy_or_confidence",
      noisyOr,
      `Noisy-OR across ${classList.length} signal class(es): ${classList.join(", ") || "(bare vendor only)"}`,
    );
