/**
 * N7 — Named VerificationStep factories.
 */
...
export function verifyTokenSource(fact: ProgressTokenFact): VerificationStep {
  ...
  return {
    ...
    instruction:
      `Open the file at ${targetOf(fact.location)} and confirm the progress token ` +
...
export function verifyNotificationsEmit(fact: ProgressTokenFact): VerificationStep {
  return {
    ...
    instruction:
      `Trace the progress token from ${targetOf(fact.location)} to the notifications/progress ` +
