import { describe, it, expect } from "vitest";
import { M7Rule } from "../index.js";
import type { AnalysisContext } from "../../../../engine.js";
import { isLocation } from "../../../location.js";
import { source as tp01 } from "../__fixtures__/true-positive-01-history-push.js";
import { source as tp02 } from "../__fixtures__/true-positive-02-context-assign.js";
import { source as tp03 } from "../__fixtures__/true-positive-03-memory-splice.js";
import { source as tn01 } from "../__fixtures__/true-negative-01-filter.js";
import { source as tn02 } from "../__fixtures__/true-negative-02-plain-local.js";

function ctx(src: string, file = "scan.ts"): AnalysisContext {
  return {
    server: { id: "s", name: "n", description: null, github_url: null },
    tools: [],
    source_code: src,
    source_files: new Map([[file, src]]),
    dependencies: [],
    connection_metadata: null,
  };
}

const rule = new M7Rule();

describe("M7 — Multi-Turn State Injection (v2)", () => {
  it("fires on conversation.history.push (TP-01)", () => {
    const fs = rule.analyze(ctx(tp01));
    expect(fs.length).toBeGreaterThanOrEqual(1);
  });
  it("fires on ctx.messages = items (TP-02)", () => {
    const fs = rule.analyze(ctx(tp02));
    expect(fs.length).toBeGreaterThanOrEqual(1);
  });
  it("fires on agent.memory splice (TP-03)", () => {
    const fs = rule.analyze(ctx(tp03));
    expect(fs.length).toBeGreaterThanOrEqual(1);
  });
  it("does NOT fire on history.filter (TN-01)", () => {
    const fs = rule.analyze(ctx(tn01));
    expect(fs).toHaveLength(0);
  });
  it("does NOT fire on unrelated .push (TN-02)", () => {
    const fs = rule.analyze(ctx(tn02));
    expect(fs).toHaveLength(0);
  });
  it("every non-impact link has structured Location", () => {
    const fs = rule.analyze(ctx(tp01));
    for (const link of fs[0].chain.links) {
      if (link.type === "impact") continue;
      expect(isLocation(link.location)).toBe(true);
    }
  });
  it("verification steps carry structured Locations", () => {
    const fs = rule.analyze(ctx(tp02));
    const steps = fs[0].chain.verification_steps ?? [];
    expect(steps.length).toBeGreaterThanOrEqual(3);
    for (const s of steps) expect(isLocation(s.target)).toBe(true);
  });
  it("respects the 0.85 confidence cap", () => {
    const fs = rule.analyze(ctx(tp01));
    for (const f of fs) expect(f.chain.confidence).toBeLessThanOrEqual(0.85);
  });
});
