...
describe("I4 — Dangerous Resource URI", () => {
  it("flags file:// URI", () => {
    expect(run("I4", ctx({ resources: [{ uri: "file:///etc/passwd", name: "passwd", description: null }] } as any)).some(x => x.rule_id === "I4")).toBe(true);
  });
  it("flags javascript: URI", () => {
    expect(run("I4", ctx({ resources: [{ uri: "javascript:alert(1)", name: "xss", description: null }] } as any)).some(x => x.rule_id === "I4")).toBe(true);
  });
  it("flags path traversal in URI", () => {
    expect(run("I4", ctx({ resources: [{ uri: "https://api.com/../../../etc/shadow", name: "shadow", description: null }] } as any)).some(x => x.rule_id === "I4")).toBe(true);
  });
...
describe("I11 — Over-Privileged Root", () => {
  it("flags root filesystem root", () => {
    expect(run("I11", ctx({ roots: [{ uri: "file:///", name: "root" }] } as any)).some(x => x.rule_id === "I11")).toBe(true);
  });
  it("flags /etc root", () => {
    expect(run("I11", ctx({ roots: [{ uri: "/etc/", name: "config" }] } as any)).some(x => x.rule_id === "I11")).toBe(true);
  });
...
describe("I12 — Capability Escalation Post-Init", () => {
  ...
  it("flags undeclared sampling", () => {
    expect(run("I12", ctx({ declared_capabilities: { sampling: false }, source_code: `handleSampling(request);` } as any)).some(x => x.rule_id === "I12")).toBe(true);
  });
});

describe("I15 — Transport Session Security", () => {
  it("flags predictable session token", () => {
    expect(run("I15", ctx({ source_code: `const sessionId = Math.random().toString(36);` })).some(x => x.rule_id === "I15")).toBe(true);
  });
...
describe("I16 — Consent Fatigue", () => {
  it("flags 12 benign + 1 dangerous", () => {
    ...
    expect(run("I16", ctx({ tools })).some(x => x.rule_id === "I16")).toBe(true);
  });
